...
from agent.model.OneStepLstmRNN import OneStepLstmRNN


class LstmDRQN(nn.Module):
    def __init__(self, load_path=None, in_dim=1, out_dim=1, hidden_dim=1, rnn_layers=2, model=None, learning_rate=1e-3,
        ...
        if model is not None:
            ...
            self.model = OneStepLstmRNN(in_dim=in_dim, out_dim=out_dim, hidden_dim=hidden_dim, rnn_layers=rnn_layers)
    ...
    def train(self, env, epsilon=1, epochs=1000, memory_size=1000, batch_size=200,
        ...
        for i in tqdm(range(epochs)):
            ...
            experiences = state1.detach().clone().to(self.device)

            finish = False
            moves_count = 0
            rewards = []
            hidden_state1 = torch.zeros(self.rnn_layers, self.hidden_dim, requires_grad=False).to(self.device)
            cell_state1 = torch.zeros(self.rnn_layers, self.hidden_dim, requires_grad=False).to(self.device)
            # hidden_state0 = hidden_state1.detach().clone()
            # cell_state0 = cell_state1.detach().clone()
            ...
            while not finish:
                sync_count += 1
                moves_count += 1
                qval, hidden_state2, cell_state2 = self.model(experiences, hidden_state1, cell_state1)
                ...
                win = env.judge_win(done, total_reward) if done else False
                ...
                qval2, hidden_state3, cell_state3 = self.model(state2, hidden_state2, cell_state2)
                Y = qval.detach().clone()

                # TODO: Optimisation (done - win) made to adapt to Cartpole game. Check if other games can use this
                Y[-1, action_] = reward + self.gamma * ((1 - (done - win)) * torch.max(qval2))
                # Y[-1, action_] = reward + self.gamma * torch.max(qval2)
                X = qval.clone()
                ...
                experiences = torch.cat((experiences, state2), dim=0).to(self.device)
                ...
                #         Q1, HS1 = self.agent(state1_batch, torch.transpose(hidden_state1_batch, 0, 1))
                #         with torch.no_grad():
                #             # Q2, HS2 = self.agent(state2_batch, torch.transpose(hidden_state2_batch, 0, 1))
                #             Q2, HS2 = self.agent(state2_batch, HS1)
                #
    ...
    def test_model(self, env, max_moves=50, display=True, noise=True):
        ...
        experiences = state.detach().clone()
        hidden_state = torch.zeros(self.rnn_layers, self.hidden_dim, requires_grad=False).to(self.device)
        cell_state = torch.zeros(self.rnn_layers, self.hidden_dim, requires_grad=False).to(self.device)
        # if display:
        #     env.render()

        while not finish:
            qval, hidden_state_, cell_state_ = self.model(experiences, hidden_state, cell_state)
            ...
            state = torch.from_numpy(state_).float().to(self.device)
            experiences = torch.cat((experiences, state), dim=0)
            rewards.append(reward)
            # if display:
            #     env.render()
            if done or moves_count >= max_moves:
