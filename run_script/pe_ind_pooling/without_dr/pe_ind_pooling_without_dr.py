...
from agent.model.layer.PESymetry import PESymetryMean, PESymetryMeanAct
...
class QNetwork(nn.Module):
    def __init__(self, input_dim: int, output_dim: int) -> None:
        ...
        pe_layers = [
            PESymetryMean(input_dim, input_dim * neuron_nb_factor),
            nn.ELU(),
            # nn.Tanh(),
            PESymetryMean(input_dim * neuron_nb_factor, input_dim * neuron_nb_factor),
            nn.ELU(),
            # PESymetryMean(in_dim * 5, in_dim * 5),
            # nn.LeakyReLU(),
            PESymetryMean(input_dim * neuron_nb_factor, output_dim),
        ]
        ...
        for round_nb in round_nbs:
            for episode in range(num_episodes):
                ...
                while not done:
                    ...
                    if episode % 10 == 0 and round_nr == int(round_nb/2):
                        # agent.old_q_network = agent.q_network
                        win_per_temp = test_agent(agent, num_episodes=10, train_epoch=episode, path="test_data.csv",
                                                  repeat=_)
